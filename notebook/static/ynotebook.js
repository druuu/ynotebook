...
window.ymq_resolve = true;
var ymap = y.define('ymap', Y.Map);
ymap.observe(function (e) {
    ymq();
    if (window.ymq_resolve) {
        window.ymq_resolve = false;
        ymq();
    }
});
window.ymap = ymap;
...
function ymq() {
    if (typeof Jupyter !== 'undefined' && typeof Jupyter.notebook !== 'undefined') {
        var queue = ymap.get('queue');
        var to = $('#notebook-container');
        for (var i=0; i<=queue.length; i++) {
            var new2 = queue[i]['new2'];
            var old = queue[i]['old'];
            if (new2 === 0) {
                to.prepend(Jupyter.notebook.get_cell(old));
            } else {
                to.children().eq(new2-1).after(Jupyter.notebook.get_cell(old));
            }
        }

        var queue2 = ymap.get('queue2');
        for (var i=0; i<=queue2.length; i++) {
            if (queue2[i]['change_to'] === 'markdown') {
                Jupyter.notebook.to_markdown(queue2[i]['index']);
            } else if (queue2[i]['change_to'] === 'code') {
                Jupyter.notebook.to_code(queue2[i]['index']);
            }
        }
    } else {
        setTimeout(set_cell, 0, old, new2);
    }
}
